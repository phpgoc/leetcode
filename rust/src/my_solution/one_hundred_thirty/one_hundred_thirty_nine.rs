...
pub fn word_break(s: String, word_dict: Vec<String>) -> bool {
    ...
    //    println!("{:?}", map);
    word_break_recursive(0, &max_len, &len, &map, &s, &mut res_map)
}
    ...
    for i in 0..do_time {
        ...
        match map.get(&s[from..=from + i]) {
            Some(t) => {
                ...
                if t.is_empty() {
                    return word_break_recursive(from + i + 1, max_len, len, map, s, res_map);
                } else {
                    let mut res = false;
                    ...
                    for j in t.iter() {
                        if (&s[from..]).starts_with(j) {
                            res =
                                word_break_recursive(from + j.len(), max_len, len, map, s, res_map);
                            if res {
                    ...
                    res = word_break_recursive(from + i + 1, max_len, len, map, s, res_map);
                    res_map[from] = Some(res);
